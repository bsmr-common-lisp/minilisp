...
miniexp_t specialform_t::call (miniexp_t args, miniexp_t env, bool)
{
...
{
  typedef
  miniexp_t (*fptr_t) (int, miniexp_t *, miniexp_t);
  fptr_t
    fptr;
  const int
    args;
...
public:
  ...
  virtual miniexp_t
  call (miniexp_t args, miniexp_t env, bool);
};
...
miniexp_t primitive_t::call (miniexp_t args, miniexp_t env, bool apply)
{
...
function_t::function_t (miniexp_t a, miniexp_t b, miniexp_t e):args (a), body (b),
env (e)
{
  check_args (a);
}

miniexp_t function_t::call (miniexp_t args, miniexp_t env, bool apply)
{
  minivar_t xargs = apply ? args : evaluate_list (args, env);
...
miniexp_t function_t::funcdef (miniexp_t name)
{
  if (name)
    {
...
macrofunction_t::macrofunction_t (miniexp_t a, miniexp_t b, miniexp_t e):function_t (a, b, e)
{
}

miniexp_t macrofunction_t::call (miniexp_t args, miniexp_t env, bool)
{
  minivar_t nenv = this->env;
...
miniexp_t macrofunction_t::funcdef (miniexp_t name)
{
  if (name)
    {
  ...
  while (miniexp_consp (v))
    {
      ...
      v = miniexp_cdr (v);
      if (!(miniexp_consp (a) &&
	    miniexp_symbolp (miniexp_car (a)) && !miniexp_cddr (a)))
	{
