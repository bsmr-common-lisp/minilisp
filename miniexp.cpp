...
#if defined(__GNUC__)
static void assertfail (const char *fn, int ln) __attribute__ ((noreturn));
#endif
...
BEGIN_ANONYMOUS_NAMESPACE class symtable_t
{
public:
  int nelems;
  int nbuckets;
...
symtable_t::symtable_t ():nelems (0), nbuckets (0), buckets (0)
{
  resize (7);
}
...
END_ANONYMOUS_NAMESPACE static symtable_t *symbols;
...
minivar_t::minivar_t (miniexp_t p):data (p)
{
  if ((next = vars))
    {
      next->pprev = &next;
...
bool miniobj_t::isa (miniexp_t) const
{
  return false;
}
...
BEGIN_ANONYMOUS_NAMESPACE class ministring_t:public miniobj_t
{
...
END_ANONYMOUS_NAMESPACE static bool
...
BEGIN_ANONYMOUS_NAMESPACE struct CompatCounter
{
  static int count;
  CompatCounter ()
  {
...
int
  CompatCounter::count = 0;

END_ANONYMOUS_NAMESPACE static int
...
BEGIN_ANONYMOUS_NAMESPACE struct printer_t
{
  int tab;
  bool dryrun;
  miniexp_io_t * io;
...
// === NOT FORMATTIED BELOW THIS LINE ==================================
...
bool printer_t::must_quote_symbol (const char *s)
{
  int
    c;
  const char *
...
END_ANONYMOUS_NAMESPACE miniexp_t
  ...
  for (;;)
      ...
      else if (c == '\\')
	{
	  ...
	  if (c == '\n')	// LF
		...
		{
		  x =
		    (x << 4) + (isdigit (c) ? c - '0' : toupper (c) - 'A' +
				10);
		  c = io->fgetc (io);
		  if (isxdigit (c))
		    {
		      x =
			(x << 4) + (isdigit (c) ? c - '0' : toupper (c) -
				    'A' + 10);
		      c = io->fgetc (io);
		    }
		  append ((char) x, s, l, m);
...
miniexp_t
miniexp_pprin (miniexp_t p, int w)
{
  return miniexp_pprin_r (&miniexp_io, p, w);
}

miniexp_t
miniexp_pprint (miniexp_t p, int w)
{
